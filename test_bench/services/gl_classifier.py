...
COL_GL_NAME  = 1   # B: GL Account Name
...
class GLClassifier:
    ...
    def validate_against_accounts(self, account_names: List[str]) -> None:
        """
        Cross-check every GL Account Name in the mapping sheet against a
        known list of accounts.  Logs a WARNING for any name that cannot
        be found so the user can fix typos.

        Args:
            account_names: List of account display names.
        """
        self._ensure_fresh()
        ...
        print(
            ...
            f"matched in reference accounts."
        )
        for name in missing:
            print(
                f"[GL] WARNING — GL name '{name}' in sheet NOT found in reference. "
                f"Fix the spelling or add it."
            )
