"""
Google Sheets Service — logs extracted invoice data, one row per line item.

Adapted from QB-Pipeline-ATMT: removed QBO columns (QBO Status, QBO Bill ID,
QBO Currency) and update_qbo_status() method.
"""
...
class GoogleSheetsService:
    ...
    # (QBO Status, QBO Bill ID, QBO Currency removed)
    HEADERS = [
        "Timestamp",
        "File ID",
        "Line #",
    ...
    def append_invoice(self, invoice_data: dict, file_id: str, filename: str) -> bool:
        ...
        try:
            ...
            for index, item in enumerate(line_items, start=1):
                ...
                # construct row matching HEADERS order (no QBO columns)
                row = [
                    timestamp,                                  # Timestamp
                    file_id,                                    # File ID
                    line_id,                                    # Line #
    ...
    def update_status(self, file_id: str, status: str, qb_transaction_id: str = None):
        ...
        try:
            # Status is Column X (24th column)
    ...
    def get_invoices(self, status_filter: str = None) -> List[Dict]:
        ...
        try:
            result = self.sheet.values().get(
                spreadsheetId=self.spreadsheet_id, range="Invoices!A:Z"
            ).execute()
            values = result.get('values', [])
